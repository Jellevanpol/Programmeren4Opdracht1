...
const mealController = {
  ...
  createMeal: (req, res, next) => {
    ...
    let sqlStatement = 'INSERT INTO `meal` (`isActive`, `isVega`, `isVegan`, `isToTakeHome`, `maxAmountOfParticipants`, `price`, `imageUrl`, `name`, `description`, `allergenes`, `cookId`) VALUES' +
      "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?); " +
      'Select * FROM user WHERE id=? '
    pool.getConnection(function (err, conn) {
      // Do something with the connection
      ...
      if (conn) {
        conn.query(sqlStatement, [isActive, meal.isVega, meal.isVegan, meal.isToTakeHome, meal.maxAmountOfParticipants, meal.price, meal.imageUrl,
